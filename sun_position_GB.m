function sun = sun_position_GB(dt, lat, lon, altitude, delta_UTC)
...
%
% Modified to input vectors by Guillaume Bourdin June 6, 2020
%
% Input parameters:
%   dt: <Nx1> datetime / datenum / datestr
%   lat: <Nx1> double latitude in decimal degrees
%   lon: <Nx1> double latitude in decimal degrees
%   altitude: <1x1> or <Nx1> double altitude
%   delta_UTC: <1x1> or <Nx1> double offset hour from UTC in hour
%
% Output parameters
...
% convert datenum or datestr into datetime
if ~isdatetime(dt)
    try
        ...
        try 
            ...
            error('Date/time format not valid')
        end
    end
end

% create table for output
sun = table(dt, lat, lon, NaN(size(dt)), NaN(size(dt)), 'VariableNames', ...
    {'dt', 'lat', 'lon', 'sunzenith', 'sunazimuth'});
...
julian = julian_calculation(dt, delta_UTC);
...
observer_local_hour = observer_local_hour_calculation(apparent_stime_at_greenwich, lon, sun_rigth_ascension);

% 12. Calculate the topocentric sun position (rigth ascension, declination and
% rigth ascension parallax in degrees)
topocentric_sun_position = topocentric_sun_position_calculate(earth_heliocentric_position, lat, altitude, observer_local_hour, sun_rigth_ascension, sun_geocentric_declination);
...
sun = sun_topocentric_zenith_angle_calculate(lat, topocentric_sun_position, topocentric_local_hour, sun);
...
function julian = julian_calculation(dt, delta_UTC)
...
%fprintf('Julian day calculation ... ')
...
M(M == 1 | M == 2) = M(M == 1 | M == 2) + 12;


ut_time = ((ho - delta_UTC)/24) + (minut/(60*24)) + (sec/(60*60*24)); % time of day in UT time. 
D = da + ut_time; % Day of month in decimal time, ex. 2sd day of month at 12:30:30UT, D=2.521180556

% In 1582, the gregorian calendar was adopted
if any(ye == 1582)
    A = NaN(size(dt,1), 1);
    B = NaN(size(dt,1), 1);
    id1582_10_4 = ye == 1582 & mo == 10 & da <= 4; % The Julian calendar ended on October 4, 1582
    B(id1582_10_4) = 0;

    id1582_10_15 = ye == 1582 & mo == 10 & da >= 15; % The Gregorian calendar started on October 15, 1582
    A(id1582_10_15) = floor(Y(id1582_10_15) / 100);
    B(id1582_10_15) = 2 - A(id1582_10_15) + floor(A(id1582_10_15)/4);

    if any(ye == 1582 & ~id1582_10_4 & ~id1582_10_15)
        fprintf('%i date never existed! Date(s) automatically set to October 4, 1582', sum(ye == 1582 & ~id1582_10_4 & ~id1582_10_15));
        mo(ye == 1582 & ~id1582_10_4 & ~id1582_10_15) = 10;
%         da(ye == 1582 & ~id1582_10_4 & ~id1582_10_15) = 4;
        B(ye == 1582 & ~id1582_10_4 & ~id1582_10_15) = 0;
    end
    id1582_inf10 = ye == 1582 & mo < 10; % Julian calendar 
    B(id1582_inf10) = 0;
    id1582_sup10 = ye == 1582 & mo > 10; % Gregorian calendar
    A(id1582_sup10) = floor(Y(id1582_sup10)/100);
    B(id1582_sup10) = 2 - A(id1582_sup10) + floor(A(id1582_sup10)/4);
    
elseif all(ye < 1582) % Julian calendar
    B = 0;
else
...
%fprintf('Earth heliocentric position calculation ... ')
...
L5 = NaN(size(JME,1), 1);
for i = 1:size(JME,1)
    L0(i) = sum(A0 .* cos(B0 + (C0 * JME(i))));
    L1(i) = sum(A1 .* cos(B1 + (C1 * JME(i))));
    L2(i) = sum(A2 .* cos(B2 + (C2 * JME(i))));
    L3(i) = sum(A3 .* cos(B3 + (C3 * JME(i))));
    L4(i) = sum(A4 .* cos(B4 + (C4 * JME(i))));
    L5(i) = A5 .* cos(B5 + (C5 * JME(i)));
end

earth_heliocentric_position.longitude = (L0 + (L1 .* JME) + (L2 .* JME.^2) + ...
    (L3 .* JME.^3) + (L4 .* JME.^4) + (L5 .* JME.^5)) / 1e8; 
...
nutation.longitude = NaN(size(JCE,1), 1);
nutation.obliquity = NaN(size(JCE,1), 1);
for i = 1:size(JCE,1)
    tabulated_argument = (Y_terms * Xi(:,i)) * pi/180;
    delta_longitude = ((nutation_terms(:,1) + (nutation_terms(:,2) * JCE(i)))) .* sin(tabulated_argument);
    delta_obliquity = ((nutation_terms(:,3) + (nutation_terms(:,4) * JCE(i)))) .* cos(tabulated_argument);

    % Nutation in longitude
    nutation.longitude(i) = sum(delta_longitude) / 36000000;

    % Nutation in obliquity
    nutation.obliquity(i) = sum(delta_obliquity) / 36000000;
end
%fprintf('Done\n')
...
mean_obliquity = p(1).*U.^10 + p(2).*U.^9 + p(3).*U.^8 + p(4).*U.^7 + p(5).*U.^6 + ...
    p(6).*U.^5 + p(7).*U.^4 + p(8).*U.^3 + p(9).*U.^2 + p(10).*U + p(11);
...
function observer_local_hour = observer_local_hour_calculation(apparent_stime_at_greenwich, lon, sun_rigth_ascension)

observer_local_hour = apparent_stime_at_greenwich + lon - sun_rigth_ascension;
% Set the range to [0-360]
observer_local_hour = set_to_range(observer_local_hour, 0, 360);


function topocentric_sun_position = topocentric_sun_position_calculate(earth_heliocentric_position, lat, altitude, observer_local_hour, sun_rigth_ascension, sun_geocentric_declination)
...
u = atan(0.99664719 * tan(lat * pi/180));

% Term x, used in the following calculations
x = cos(u) + ((altitude/6378140) .* cos(lat * pi/180));

% Term y, used in the following calculations
y = (0.99664719 * sin(u)) + ((altitude/6378140) .* sin(lat * pi/180));
...
function sun = sun_topocentric_zenith_angle_calculate(lat, topocentric_sun_position, topocentric_local_hour, sun)
...
argument = (sin(lat * pi/180) .* sin(topocentric_sun_position.declination * pi/180)) + ...
    (cos(lat * pi/180) .* cos(topocentric_sun_position.declination * pi/180) .* cos(topocentric_local_hour * pi/180));
...
sun.sunzenith = 90 - apparent_elevation;
...
denominator = (cos(topocentric_local_hour * pi/180) .* sin(lat * pi/180)) - ...
    (tan(topocentric_sun_position.declination * pi/180) .* cos(lat * pi/180));
sun.sunazimuth = (atan2(nominator, denominator) * 180/pi) + 180;
% Set the range to [0-360]
sun.sunazimuth = set_to_range(sun.sunazimuth, 0, 360);
...
if any(var<min_interval)
    var(var<min_interval) = var + max_interval;
end
